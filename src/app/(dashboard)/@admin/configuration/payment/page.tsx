...
export default function Payment() {
  ...
  // const { data: packagesData, isLoading: packagesLoading } = useGetSubscriptionPackagesQuery();
  // const { data: currentSubscription, isLoading: subscriptionLoading } = useGetCurrentSubscriptionQuery();
  ...
  const handleSelectPackage = (packageId: string) => {
    ...
    const selectedPkg = subscriptionPackages.find(pkg => pkg.id === packageId);
    if (selectedPkg) {
  ...
  const handleCheckoutSuccess = () => {
    ...
    window.location.href = "/payment/success?reference=custom_checkout_" + Date.now();
  };
    ...
    <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Expires: {new Date(currentSubscriptionInfo.expiresAt).toLocaleDateString()}
                </p>
              </div>
              <Badge variant={currentSubscriptionInfo.isActive ? "default" : "destructive"}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {subscriptionPackages.map((pkg) => (
          <Card 
            ...
            className={`
              flex flex-col
              ${pkg.id === currentSubscriptionInfo?.packageId ? 'border-primary ring-1 ring-primary' : ''}
          ...
          >
            ...
            <CardFooter>
              <Button 
                ...
                className={`w-full ${selectedPackage === pkg.id ? 'bg-primary' : 'bg-secondary'}`}
                disabled={pkg.id === currentSubscriptionInfo?.packageId && currentSubscriptionInfo?.isActive}
              >
                {pkg.id === currentSubscriptionInfo?.packageId && currentSubscriptionInfo?.isActive
                  ? "Current Plan"
      ...
      {!showCustomCheckout && (
        <Card className="mt-4 bg-amber-50 border-amber-200">
          ...
          <CardContent>
            <p className="text-sm">
              This payment will be processed securely through Paystack. After successful payment, your subscription will be automatically activated.
              For any payment issues, please contact support.
            </p>
          </CardContent>
        </Card>
      )}
