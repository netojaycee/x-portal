...
import {
  ...
  useGetSessionsPublicQuery, // New query for sessions
...
const AdmissionPublicFormSchema = z
  .object({
    // Student Info
...
type AdmissionPublicFormValues = z.infer<typeof AdmissionPublicFormSchema>;
...
interface AdmissionPublicFormProps {
  admissionId?: string;
  isEditMode?: boolean;
  // School header props
  schoolHeader?: SchoolHeaderProps;
  schoolId?: string; // Optional school ID for the form submission
}

const AdmissionPublicForm: React.FC<AdmissionPublicFormProps> = ({
  admissionId,
  isEditMode = false,
  schoolHeader = {
    ...
    leftLogoAlt: "School Logo Left",
    rightLogoAlt: "School Logo Right",
  ...
  const { data: sessionsData, isLoading: sessionsLoading } = useGetSessionsPublicQuery({
    schoolId
  }, { 
    skip: !schoolId 
  });
  
  const { data: classesData, isLoading: classesLoading } = useGetClassesPublicQuery({
    schoolId
  }, { 
    skip: !schoolId 
  });
  ...
  const [createAdmissionPublic, { isLoading: isCreating }] =
    useCreateAdmissionPublicMutation();
  // const [updateAdmission, { isLoading: isUpdating }] = useUpdateAdmissionMutation();

  // Initialize the form with React Hook Form
  const form = useForm<AdmissionPublicFormValues>({
    resolver: zodResolver(AdmissionPublicFormSchema),
    defaultValues: {
      // Student Info
      firstname: "",
      lastname: "",
  ...
  const onSubmit = async (values: AdmissionPublicFormValues) => {
    try {
      // Restructured payload following the exact format provided
      const submissionData = {
        sessionId: values.sessionId,
        schoolId, // You might need to get this from context or props
      ...
      if (isEditMode && admissionId) {
        ...
        await createAdmissionPublic(submissionData).unwrap();
      ...
      <div className='bg-white p-6 border-b'>
        <div className='flex items-center justify-between max-w-3xl mx-auto'>
          ...
          <div className='flex-shrink-0'>
            <Image
              src={schoolHeader.leftLogoSrc || "/school-logo.png"}
              alt={schoolHeader.leftLogoAlt || "School Logo"}
              width={80}
              height={80}
          ...
          <div className='text-center flex-1 mx-6'>
            <h1
              ...
              style={{ color: schoolHeader.schoolNameColor || "#E91E63" }}
            >
              {schoolHeader.schoolName || "DEEPER LIFE HIGH SCHOOL"}
            </h1>
            <div className='text-gray-600 text-sm space-y-1'>
              ...
              <p>
                E-Mail: {schoolHeader.email || "info@deeperlifehighschool.org"}
              </p>
              <p>Tel: {schoolHeader.phone || "08033319011,08035540255"}</p>
            </div>
          </div>
          ...
          <div className='flex-shrink-0'>
            <Image
              src={schoolHeader.rightLogoSrc || "/school-logo.png"}
              alt={schoolHeader.rightLogoAlt || "School Logo"}
              width={80}
              height={80}
              className='w-20 h-20 object-contain'
            />
                  ...
                  <Label htmlFor='image-upload' className='mt-4'>
                    <div className='border-primary text-primary hover:border-primary/70 border-2 rounded-md flex items-center p-2 space-x-2'>
                      <Image
                        ...
                        className=''
                      />{" "}
                      <span className='text-xs'>Upload Student Image</span>
                    </div>
                  </Label>
                  <Input
                    id='image-upload'
...
export default AdmissionPublicForm;
